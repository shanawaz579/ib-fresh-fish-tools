...
BEGIN
  ...
  IF coalesce(p_payment_amount, 0) > 0 THEN
    PERFORM working.record_purchase_bill_payment(
      ...
      p_payment_reference, 'Initial payment recorded with corrected bill'
    );
    SELECT * INTO replacement_bill
...
REVOKE ALL ON FUNCTION working.revise_purchase_bill_with_source_changes(
  BIGINT, TEXT, BIGINT, DATE, JSONB, NUMERIC, NUMERIC, VARCHAR, TEXT, NUMERIC, NUMERIC, TEXT, TEXT
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION working.revise_purchase_bill_with_source_changes(
  BIGINT, TEXT, BIGINT, DATE, JSONB, NUMERIC, NUMERIC, VARCHAR, TEXT, NUMERIC, NUMERIC, TEXT, TEXT
) TO authenticated;

REVOKE EXECUTE ON FUNCTION working.revise_purchase_bill_with_payment(
  BIGINT, TEXT, BIGINT, DATE, JSONB, NUMERIC, NUMERIC, VARCHAR, TEXT, NUMERIC, NUMERIC, TEXT, TEXT
) FROM authenticated;

COMMENT ON FUNCTION working.revise_purchase_bill_with_source_changes(
  BIGINT, TEXT, BIGINT, DATE, JSONB, NUMERIC, NUMERIC, VARCHAR, TEXT, NUMERIC, NUMERIC, TEXT, TEXT
) IS 'Atomically corrects source purchase item/quantity, stock movement, bill values and initial payment with a full before-state audit.';
