...
import { useItemActivity, type ActivityPeriod } from '../features/itemActivity/useItemActivity';
...
export default function ItemsByCustomerScreen() {
  ...
  return <View style={styles.container}>
    ...
    <ScrollView refreshControl={<RefreshControl refreshing={activity.refreshing} onRefresh={activity.refresh}/>} contentContainerStyle={styles.content}>
      ...
      <View style={styles.rangeRow}>{([1,7,30,'date'] as ActivityPeriod[]).map(period=><TouchableOpacity key={period} style={[styles.rangeChip,activity.period===period&&styles.rangeChipActive]} onPress={()=>activity.selectPeriod(period)}><Text style={[styles.rangeText,activity.period===period&&styles.rangeTextActive]}>{period===1?'Today':period==='date'?'Date':`${period} days`}</Text></TouchableOpacity>)}</View>
      {activity.period === 'date' ? <View style={styles.dateNavigator}><DateNavigator date={activity.selectedDate} onPrevious={activity.previousDate} onNext={activity.nextDate} onToday={activity.goToToday} canGoNext={activity.selectedDate < activity.today} accentColor="#0F766E" /></View> : <Text style={styles.periodText}>{formatBusinessDate(activity.fromDate)} – {formatBusinessDate(activity.toDate)}</Text>}
      ...
      {activity.loading?<ActivityIndicator color="#0F766E" style={styles.loader}/>:<>
        ...
        </>:<View style={styles.empty}><Text style={styles.emptyIcon}>⌕</Text><Text style={styles.emptyTitle}>No activity {activity.period === 'date' ? 'on this date' : 'in this period'}</Text><Text style={styles.emptyText}>{activity.period === 'date' ? 'Choose another date or switch to a longer period.' : 'Try a longer period or clear a filter.'}</Text></View>}
      </>}
    </ScrollView>
